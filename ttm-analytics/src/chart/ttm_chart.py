...
def draw_line_chart(data_frame: pd.DataFrame, timestamp_column_name: str, value_column_name: str,
    ...
    plot_x_ticks_count = 40
    plot_x_ticks = [min_timestamp + round(i * (max_timestamp - min_timestamp) / float(plot_x_ticks_count))
                    for i in range(0, plot_x_ticks_count + 1)]
    ...
    min_value = sampled_data_frame[value_column_name].min()
    max_value = sampled_data_frame[value_column_name].max()
    plot_y_ticks_count = 20
    plot_y_ticks = [min_value + round(i * (max_value - min_value) / float(plot_y_ticks_count))
                    for i in range(0, plot_y_ticks_count + 1)]
    plot.set_yticks(plot_y_ticks)
    plot.set_yticklabels([f"{plot_y_tick:,.2f}" for plot_y_tick in plot_y_ticks])
